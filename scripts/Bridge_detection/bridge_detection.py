...
class bridge_detection:
	...
	def segment_window(self,cv_image):
		...
		# print("edges shape = ",edges.shape, edges[30:40,300:310])
		edges = cv2.adaptiveThreshold(edges,255,cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY,11,3)
		# edges = np.array(edges)
		# kernel = (53,53)#cv2.getStructuringElement(cv2.MORPH_RECT,(13,13))
		...
		# print(np.dtype(dilation))
		# dilation = np.invert(dilation)
		# minLength = 80 
		# maxLineGap = 40
		# houghlines_gray, contours, hierarchy = cv2.findContours(edges, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
		# print("len contours",len(contours))
		# # cv2.imshow("houghlines", houghlines_gray)
		# # cv2.waitKey(0)
		# epsilon = 0.1*cv2.arcLength(contours[0], True)
		# approx = cv2.approxPolyDP(contours[0], epsilon, True)
		# edges3d = np.dstack((edges,edges,edges))
		# if ((len(approx) >= 4)):
		# 	points = np.squeeze(np.array(approx))
		# 	if cv2.isContourConvex(points):
		# 		# corners = self.get_outer_window_corner_points(points, edges)
		# 		cv2.drawContours(edges3d, [approx], 10, (0,255,0), 3)
		...
		# edges3d = np.uint8(edges3d)
		# cv2.imshow("contour", edges3d)
		# cv2.waitKey(0)
		# closed = np.invert(closed)
		# lines = cv2.HoughLinesP(closed,20, np.pi/90, 80, minLength, maxLineGap)
		# # lines = cv2.HoughLines(gray, 1, np.pi/180, 10, None, 0, 0 )
		# print lines.shape

		# # # input('as')
		# houghlines = closed.copy()
